...
class ContractService extends BaseMaintenanceService
{
    ...
    public function createContract(array $data): MaintenanceContract
    {
        ...
        if (!$this->validateMaintenancePermissions('create_contract')) {
            throw new \InvalidArgumentException('You do not have permission to create maintenance contracts');
        }

        DB::beginTransaction();
        
        try {
            // Determine branch_id first
            ...
            if ($branchId && !$this->validateBranchAccess($branchId)) {
                throw MaintenanceException::branchAccessDenied($branchId);
            }
    ...
    public function updateContract(int $id, array $data): MaintenanceContract
    {
        ...
        if (!$this->validateMaintenancePermissions('update_contract')) {
            throw new \InvalidArgumentException('You do not have permission to update maintenance contracts');
        }
        ...
        if (!$this->validateTenantIsolation($contract->tenant_id)) {
            throw new \InvalidArgumentException('You do not have access to this contract');
        }
        ...
        try {
            ...
            if (isset($data['branch_id']) && $data['branch_id'] && !$this->validateBranchAccess($data['branch_id'])) {
