...
class BusinessSeeder extends Seeder
{
    ...
    private function createTenantStaff(Tenant $tenant): void
    {
        ...
        if ($existingUsers->isNotEmpty()) {
            // Assign roles to existing users and attach to branch
            foreach ($existingUsers as $index => $user) {
                ...
                // Attach user to default branch
                if (!$user->branches()->where('branch_id', $defaultBranch->id)->exists()) {
                    $user->branches()->attach($defaultBranch->id, [
                        'is_manager' => $index <= 1, // Admin and manager are managers
                    ]);
                }
            }
            return;
        ...
        $admin->branches()->attach($defaultBranch->id, ['is_manager' => true]);
        ...
        $manager->branches()->attach($defaultBranch->id, ['is_manager' => true]);
        ...
        for ($i = 1; $i <= 2; $i++) {
            ...
            $salesman->branches()->attach($defaultBranch->id, ['is_manager' => false]);
        }
    }
